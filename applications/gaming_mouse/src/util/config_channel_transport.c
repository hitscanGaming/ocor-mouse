...
static int data_len_check(size_t event_data_len)
{
	...
	if (event_data_len > max_size - min_size) {
		LOG_WRN("Unsupported event data length %" PRIu8, event_data_len);
		return -EMSGSIZE;
	}
...
int config_channel_report_parse(const uint8_t *buffer, size_t length, struct config_event *event)
{
	...
	if (event->dyndata.size < config_data_len) {
		LOG_ERR("Invalid packet size %zu < %zu", event->dyndata.size, config_data_len);
		return -ENOMEM;
	}
...
{
	struct config_channel_transport *transport =
		CONTAINER_OF(work, struct config_channel_transport, timeout.work);
...
int config_channel_transport_get(struct config_channel_transport *transport, uint8_t *buffer,
				 size_t length)
{
	__ASSERT_NO_MSG(transport->state != CONFIG_CHANNEL_TRANSPORT_DISABLED);
...
int config_channel_transport_set(struct config_channel_transport *transport, const uint8_t *buffer,
				 size_t length)
{
	__ASSERT_NO_MSG(transport->state != CONFIG_CHANNEL_TRANSPORT_DISABLED);
	...
	if (transport->state == CONFIG_CHANNEL_TRANSPORT_RSP_READY) {
		LOG_WRN("Host ignored previous response (transport: %p)", (void *)transport);
	}

	struct config_event *event = new_config_event(length - TRANSPORT_HEADER_SIZE);
					  ...
					  struct config_event *event)
{
	...
	int pos = config_channel_report_fill(transport->data,
					     event->dyndata.size + TRANSPORT_HEADER_SIZE, event);
