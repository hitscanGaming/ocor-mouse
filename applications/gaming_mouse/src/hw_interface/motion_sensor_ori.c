...
static const char *const opt_descr[] = {[SENSOR_OPT_VARIANT] = OPT_DESCR_MODULE_VARIANT,
					[SENSOR_OPT_CPI] = "cpi",
...
static void data_ready_handler(const struct device *dev, const struct sensor_trigger *trig)
{
	...
	case STATE_DISCONNECTED:
		/* Wake up thread */
		k_sem_give(&sem);
		LOG_ERR("k_sem_give, data readt hadler STATE_DISCONNECTED");
...
static void motion_thread_fn(void)
{
	...
	while (!err) {
		...
		LOG_ERR("state=%d, send_event=%d, sample=%d", state.state, send_event,
			state.sample);
		state.sample = false;
		...
		if (IS_ENABLED(CONFIG_DESKTOP_CONFIG_CHANNEL_ENABLE) && !err &&
		    unlikely(option_bm)) {
			write_config();
		}
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (is_hid_report_subscription_event(aeh)) {
		...
		if ((event->report_id == REPORT_ID_MOUSE) ||
			...
			k_sem_give(&sem);
			LOG_ERR("k_sem_give, peer_count=%d, state=%d", state.peer_count,
				state.state);
		}
	...
	if (IS_ENABLED(CONFIG_DESKTOP_MOTION_PM_EVENTS) && is_wake_up_event(aeh)) {
		...
		if ((state.state == STATE_SUSPENDED) ||
			...
			k_sem_give(&sem);
			LOG_ERR("k_sem_give, wake up event, state=%d", state.state);

			module_set_state(MODULE_STATE_READY);
		} else if (state.state == STATE_DISABLED_SUSPENDED) {
			state.state = STATE_DISABLED;
		}
		LOG_ERR("wake up event, state=%d", state.state);
