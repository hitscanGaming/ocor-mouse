...
static bool update_digit_input_key(struct input_buf *input, uint16_t in_key_id, bool pressed)
{
	if (!pressed) {
		return false;
	}
...
static bool update_delete_key(struct input_buf *input, uint16_t in_key_id, bool pressed)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(delete_keys); i++) {
		if (in_key_id == delete_keys[i]) {
			if (pressed) {
				...
				int64_t hold_time = k_uptime_delta(&delete_press_times[i]);
...
static bool update_confirm_key(struct input_buf *input, uint16_t in_key_id, bool pressed)
{
	if (!pressed) {
		return false;
	}
...
static bool button_event_handler(const struct button_event *event)
{
	...
	static bool (*update_key_func[])(struct input_buf *input, uint16_t in_key_id,
					 bool pressed) = {update_digit_input_key,
							  update_confirm_key, update_delete_key};

	for (int i = 0; i < ARRAY_SIZE(update_key_func); i++) {
		if ((*update_key_func[i])(&digits, event->key_id, event->pressed)) {
			break;
		}
	}
...
static void init(void)
{
	...
	BUILD_ASSERT(CONFIG_DESKTOP_PASSKEY_MAX_LEN != 0, "Passkey length not set in Kconfig");
	BUILD_ASSERT(sizeof(confirm_keys) > 0, "No confirm key defined");
