...
static int port_setup(const struct device *port, const struct pin_state *pin_state, size_t cnt)
{
	int err = 0;
	...
	for (size_t i = 0; i < cnt; i++) {
		err = gpio_pin_configure(port, pin_state[i].pin, GPIO_OUTPUT);
		if (!err) {
			err = gpio_pin_set_raw(port, pin_state[i].pin, pin_state[i].val);
		} else {
			LOG_ERR("Cannot configure pin %u on %s", pin_state[i].pin, port->name);
			break;
		}
	}
...
static int ports_setup(const struct port_state *port_state, size_t cnt)
{
	...
	for (size_t i = 0; i < cnt; i++) {
		err = port_setup(port_state[i].port, port_state[i].ps, port_state[i].ps_count);
		if (err) {
			break;
		}
	}
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (IS_ENABLED(CONFIG_DESKTOP_BOARD_PM_EVENTS) && is_wake_up_event(aeh)) {
		if (!initialized) {
			initialized = true;
	...
	if (IS_ENABLED(CONFIG_DESKTOP_BOARD_PM_EVENTS) && is_power_down_event(aeh)) {
