...
static int read_state(struct selector *selector)
{
	...
	for (size_t i = 0; i < selector->config->pins_size; i++) {
		value = gpio_pin_get_raw(gpio_dev[sel_pins[i].port], sel_pins[i].pin);
		if (value < 0) {
			LOG_ERR("Cannot read value from port:%" PRIu8 " pin: %" PRIu8,
				sel_pins[i].port, sel_pins[i].pin);
			break;
		}
	...
	if (value >= 0) {
		if (value) {
			...
			LOG_WRN("Selector %" PRIu8 " position is unknown", selector->config->id);
		}
	}
...
static int enable_interrupts_lock(struct selector *selector)
{
	...
	for (size_t i = 0; i < selector->config->pins_size; i++) {
		...
		err = gpio_pin_interrupt_configure(gpio_dev[sel_pins[i].port], sel_pins[i].pin,
						   GPIO_INT_LEVEL_HIGH);
		if (err) {
...
static void selector_isr(const struct device *dev, struct gpio_callback *cb, uint32_t pins_mask)
{
	struct selector *sel;
	size_t port;
	uint8_t *ptr;
	...
	sel = CONTAINER_OF((struct gpio_callback(*)[])ptr, struct selector, gpio_cb);
...
static int configure_callbacks(struct selector *selector)
{
	...
	for (size_t i = 0; i < selector->config->pins_size; i++) {
		...
		__ASSERT_NO_MSG(sel_pins[i].pin < __CHAR_BIT__ * sizeof(bitmask[0]));
	...
	for (size_t i = 0; (i < ARRAY_SIZE(gpio_dev)) && !err; i++) {
		...
		gpio_init_callback(&selector->gpio_cb[i], selector_isr, bitmask[i]);
...
static int configure_pins(struct selector *selector, bool enabled)
{
	...
	for (size_t i = 0; (i < selector->config->pins_size) && !err; i++) {
		err = gpio_pin_configure(gpio_dev[sel_pins[i].port], sel_pins[i].pin, flags);
	}
...
static int init(void)
{
	...
	BUILD_ASSERT(ARRAY_SIZE(selector_config) > 0, "There is no active selector");
