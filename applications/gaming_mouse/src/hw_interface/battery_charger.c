...
static const struct gpio_dt_spec enable_gpio = GPIO_DT_SPEC_GET(BATTERY_CHARGER, enable_gpios);
static const struct gpio_dt_spec cso_gpio = GPIO_DT_SPEC_GET(BATTERY_CHARGER, cso_gpios);
...
static void error_check_handler(struct k_work *work)
{
	...
	if (cnt <= CSO_CHANGE_MAX) {
		...
		if (val < 0) {
			...
			state = (val == 0) ? (BATTERY_STATE_CHARGING) : (BATTERY_STATE_IDLE);
		}
	} else {
...
static void cs_change(const struct device *gpio_dev, struct gpio_callback *cb, uint32_t pins)
{
...
static int start_battery_state_check(void)
{
	...
	if (!err) {
		...
		err = gpio_pin_interrupt_configure_dt(&cso_gpio, GPIO_INT_EDGE_BOTH);
	}
