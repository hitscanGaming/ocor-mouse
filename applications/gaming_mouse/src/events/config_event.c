...
static void log_config_event(const struct app_event_header *aeh)
{
	...
	APP_EVENT_MANAGER_LOG(aeh, "%s %s rcpt: %02x id: %02x", status_str,
			      event->is_request ? "req" : "rsp", event->recipient, event->event_id);
}

APP_EVENT_TYPE_DEFINE(config_event, log_config_event, NULL,
		      APP_EVENT_FLAGS_CREATE(IF_ENABLED(CONFIG_DESKTOP_INIT_LOG_CONFIG_EVENT,
