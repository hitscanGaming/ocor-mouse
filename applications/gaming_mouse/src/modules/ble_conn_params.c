...
static void update_peer_conn_params(struct connected_peer *peer)
{
	...
	if (err) {
		LOG_WRN("Cannot update conn parameters for peer %p (err:%d)", (void *)peer->conn,
			err);
		...
		LOG_INF("Update conn params for peer: %p (%s, requested latency: %" PRIu16
			", USB suspended: %s)",
			(void *)peer->conn, peer->use_llpm ? "LLPM" : "BLE",
			peer->requested_latency,
			IS_ENABLED(CONFIG_DESKTOP_BLE_USB_MANAGED_CI) && usb_suspended ? "true"
										       : "false");
	}
}
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (is_ble_peer_conn_params_event(aeh)) {
		ble_peer_conn_params_event_handler(cast_ble_peer_conn_params_event(aeh));

		return false;
	}

	if (IS_ENABLED(CONFIG_DESKTOP_BLE_USB_MANAGED_CI) && is_usb_state_event(aeh)) {
