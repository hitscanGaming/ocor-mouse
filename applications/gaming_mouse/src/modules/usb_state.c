 ...
 *
 * Implements: REQ-USB-001
 * (USB HID device state + descriptor exposure; conformance per HID spec)
 */
#include <stdio.h>
#include <stdint.h>
...
#define _USB_HID_BUF_SIZE                                                                          \
	(ROUND_UP(REPORT_ID_SIZE + REPORT_BUFFER_SIZE_INPUT_REPORT, UDC_BUF_GRANULARITY))
#else
...
static void usb_hid_buf_send(struct usb_hid_device *usb_hid, struct usb_hid_buf *buf)
{
	...
	// LOG_INF("Sending report ID %u, size %zu", report_id, size);
...
static void report_sent(struct usb_hid_device *usb_hid, struct usb_hid_buf *buf, bool error)
{
	...
	if (next_buf) {
		// LOG_INF("Next buffer is pending to send");
...
static void update_usb_hid(struct usb_hid_device *usb_hid, bool enabled)
{
	...
	if (IS_ENABLED(CONFIG_DESKTOP_CONFIG_CHANNEL_ENABLE) && (usb_hid == &usb_hid_device[0]) &&
	    !usb_hid->enabled) {
...
static void protocol_change(const struct device *dev, uint8_t protocol)
{
	...
	if ((protocol != HID_PROTOCOL_BOOT) && (protocol != HID_PROTOCOL_REPORT)) {
		__ASSERT_NO_MSG(false);
		return;
	}
...
{
	BUILD_ASSERT(REPORT_ID_COUNT <= sizeof(usb_hid_device[0].report_bm) * CHAR_BIT);
#if CONFIG_DESKTOP_USB_SELECTIVE_REPORT_SUBSCRIPTION
	BUILD_ASSERT(ARRAY_SIZE(usb_hid_report_bm) == ARRAY_SIZE(usb_hid_device));
...
static int set_report_legacy(const struct device *dev, struct usb_setup_packet *setup, int32_t *len,
			     uint8_t **data)
{
	__ASSERT_NO_MSG(dev);
	__ASSERT_NO_MSG(setup);
					   ...
					   const struct device *dev, uint32_t report_bm)
{
	...
	if (IS_ENABLED(CONFIG_USB_HID_BOOT_PROTOCOL)) {
		...
		if (err) {
			LOG_ERR("usb_hid_set_proto_code failed for dev: %p (err: %d)", (void *)dev,
				err);
			return err;
		}
	}
...
static int usb_init_next_hid_device_init(struct usb_hid_device *usb_hid_dev, uint32_t report_bm)
{
	...
	if (err) {
		LOG_ERR("hid_device_register failed for %p (err: %d)", (void *)usb_hid_dev->dev,
			err);
	}
...
static int usb_init_next_register_fs_classes(struct usbd_context *usbd)
{
	...
	STRUCT_SECTION_FOREACH_ALTERNATE(usbd_class_fs, usbd_class_node, c_nd)
	{
		...
		if (err) {
			LOG_ERR("usbd_register_class failed for %s (err: %d)", c_nd->c_data->name,
				err);
			return err;
		}
	}
...
static int usb_init_next_add_configuration(struct usbd_context *usbd, const enum usbd_speed speed,
...
static struct usbd_context *usb_init_next_usbd_init(void)
{
	...
	USBD_DEVICE_DEFINE(usbd, DEVICE_DT_GET(USB_DT_NODELABEL), CONFIG_DESKTOP_DEVICE_VID,
	...
	static const uint8_t attributes =
		IS_ENABLED(CONFIG_DESKTOP_USB_REMOTE_WAKEUP) ? (USB_SCD_REMOTE_WAKEUP) : (0);
...
static int usb_init(void)
{
	...
	if (IS_ENABLED(CONFIG_DESKTOP_USB_STACK_NEXT)) {
		err = usb_init_next();
		// LOG_INF("init usb next");
	} else {
		__ASSERT_NO_MSG(IS_ENABLED(CONFIG_DESKTOP_USB_STACK_LEGACY));
		err = usb_init_legacy();
		// LOG_INF("init usb legacy");
	}
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (IS_ENABLED(CONFIG_DESKTOP_CONFIG_CHANNEL_ENABLE) && is_config_event(aeh)) {
		config_channel_transport_rsp_receive(&cfg_chan_transport, cast_config_event(aeh));

		return false;
	}

	if (IS_ENABLED(CONFIG_DESKTOP_USB_REMOTE_WAKEUP) && is_wake_up_event(aeh)) {
		usb_wakeup();
		return false;
	}
