 ...
 *
 * Implements: REQ-DFU-001, REQ-DFU-002
 * (MCUboot-backed DFU over SMP/mcumgr; image-integrity check enables revert)
 */
					...
					size_t data_size)
{
	...
	if (data_size != sizeof(*cmd_recv)) {
		LOG_ERR("Invalid data size in recv cb: %zu (expected: %zu)", data_size,
			sizeof(*cmd_recv));
		*rc = MGMT_ERR_EUNKNOWN;
		return MGMT_CB_ERROR_RC;
	}
	...
	LOG_DBG("MCUmgr SMP Command Recv Event: group_id=%d cmd_id=%d", cmd_recv->group,
		cmd_recv->id);
...
static void dfu_backend_init(void)
{
	...
	if (!err) {
		if (is_semver_supported) {
			...
			if (release_type) {
				LOG_INF("SUIT manifest version: %d.%d.%d-%s%d", version.major,
					version.minor, version.patch, release_type,
					version.pre_release_number);
			} else {
				LOG_INF("SUIT manifest version: %d.%d.%d", version.major,
...
{
	if (IS_ENABLED(CONFIG_MCUMGR_TRANSPORT_BT) && is_ble_smp_transfer_event(aeh)) {
