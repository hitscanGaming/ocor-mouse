...
static const struct device *gpio_dev = gpio_devs[CONFIG_DESKTOP_NRF_PROFILER_SYNC_GPIO_PORT];
...
{
	sync_event_id = nrf_profiler_register_event_type(SYNC_EVENT_NAME, NULL, NULL, 0);
}

static void gen_sync_event_isr(const struct device *gpio_dev, struct gpio_callback *cb,
			       uint32_t pins)
{
	profile_sync_event();
}
...
static void gen_sync_event_fn(struct k_work *work)
{
	...
	int err = gpio_pin_set_raw(gpio_dev, (gpio_pin_t)CONFIG_DESKTOP_NRF_PROFILER_SYNC_GPIO_PIN,
				   pin_value);
	...
	if (err) {
		...
		profile_sync_event();
		k_work_reschedule(&gen_sync_event, K_MSEC(sync_period));
	}
}
...
static int init(void)
{
	...
	if (IS_ENABLED(CONFIG_DESKTOP_NRF_PROFILER_SYNC_CENTRAL)) {
		...
		if (!err) {
			...
			k_work_reschedule(&gen_sync_event, K_MSEC(SYNC_PERIOD_MIN));
		}
	} else {
		...
		if (!err) {
			err = gpio_pin_interrupt_configure(gpio_dev, pin, GPIO_INT_EDGE_BOTH);
		}
	}
