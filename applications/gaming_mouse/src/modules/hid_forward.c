...
static int settings_set(const char *key, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!strcmp(key, PERIPHERAL_ADDRESSES_STORAGE_NAME)) {
		ssize_t len = read_cb(cb_arg, &peripheral_address, sizeof(peripheral_address));
...
static int store_peripheral_address(void)
{
	if (IS_ENABLED(CONFIG_SETTINGS)) {
		...
		int err = settings_save_one(key, peripheral_address, sizeof(peripheral_address));

		if (err) {
			LOG_ERR("Problem storing peripheral addresses (err %d)", err);
			return err;
		}
	}
...
static void forward_hid_report(struct hids_peripheral *per, uint8_t report_id, const uint8_t *data,
			       size_t size)
{
	__ASSERT_NO_MSG((report_id != REPORT_ID_RESERVED) && (report_id < REPORT_ID_COUNT));
...
static uint8_t hogp_read(struct bt_hogp *hids_c, struct bt_hogp_rep_info *rep, uint8_t err,
			 const uint8_t *data)
{
...
static int register_peripheral(struct bt_gatt_dm *dm, const uint8_t *hwid, size_t hwid_len)
{
...
static void submit_forward_error_rsp(struct hids_peripheral *per, enum config_status rsp_status)
{
...
static uint8_t hogp_read_cfg(struct bt_hogp *hogp, struct bt_hogp_rep_info *rep, uint8_t err,
			     const uint8_t *data)
{
...
{
	struct hids_peripheral *per = CONTAINER_OF(work, struct hids_peripheral, read_rsp.work);
	struct bt_hogp_rep_info *config_rep =
		bt_hogp_rep_find(&per->hogp, BT_HIDS_REPORT_TYPE_FEATURE, REPORT_ID_USER_CONFIG);
...
static struct config_event *generate_response(const struct config_event *event, size_t dyndata_size)
{
...
static void send_nodata_response(const struct config_event *event, enum config_status rsp_status)
{
...
{
	BUILD_ASSERT(ARRAY_SIZE(peripherals) + CFG_CHAN_BASE_ID <= CFG_CHAN_UNUSED_PEER_ID);
	...
	case CONFIG_STATUS_INDEX_PEERS:
		for (size_t i = 0; i < ARRAY_SIZE(peripherals); i++) {
			if (is_peripheral_connected(&peripherals[i])) {
				peripherals[i].cfg_chan_id = CFG_CHAN_BASE_ID + i;
			}
		}
	...
	case CONFIG_STATUS_GET_PEER: {
		...
		struct config_event *rsp = generate_response(event, HWID_LEN + sizeof(uint8_t));
		size_t pos = 0;

		if (per) {
			memcpy(&rsp->dyndata.data[pos], per->hwid, sizeof(per->hwid));
			pos += sizeof(per->hwid);
	...
	case CONFIG_STATUS_GET_PEERS_CACHE: {
		...
		struct config_event *rsp =
			generate_response(event, CONFIG_CHANNEL_FETCHED_DATA_MAX_SIZE);
...
static bool handle_config_event(struct config_event *event)
{
	...
	config_rep = bt_hogp_rep_find(recipient_hogp, BT_HIDS_REPORT_TYPE_OUTPUT,
				      REPORT_ID_USER_CONFIG_OUT);

	if (!config_rep) {
		config_rep = bt_hogp_rep_find(recipient_hogp, BT_HIDS_REPORT_TYPE_FEATURE,
	...
	int pos = config_channel_report_fill(report, REPORT_SIZE_USER_CONFIG, event);
	...
	if (has_out_report) {
		err = bt_hogp_rep_write_wo_rsp(recipient_hogp, config_rep, report, sizeof(report),
					       hogp_write_cb);
	} else {
		err = bt_hogp_rep_write(recipient_hogp, config_rep, hogp_write_cb, report,
					sizeof(report));
	}
...
static void forward_empty_hid_report(struct hids_peripheral *per,
				     const struct bt_hogp_rep_info *rep, uint8_t report_id)
{
	...
	for (size_t orep_idx = 0; orep_idx < ARRAY_SIZE(sub->out_reports); orep_idx++) {
		if (sub->saved_out_reports_bm & BIT(orep_idx)) {
			...
			int err = send_hid_out_report(&per->hogp,
						      (uint8_t *)&sub->out_reports[orep_idx], size);
...
static int send_hid_out_report(struct bt_hogp *hogp, const uint8_t *data, size_t size)
{
	...
	struct bt_hogp_rep_info *out_rep =
		bt_hogp_rep_find(hogp, BT_HIDS_REPORT_TYPE_OUTPUT, report_id);
	...
	int err =
		bt_hogp_rep_write_wo_rsp(hogp, out_rep, &data[1], size - 1, hogp_out_rep_write_cb);
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (is_ble_discovery_complete_event(aeh)) {
		...
		register_peripheral(event->dm, event->hwid, sizeof(event->hwid));
	...
	if (is_ble_peer_event(aeh)) {
		...
		if (event->state == PEER_STATE_DISCONNECTED) {
			for (size_t i = 0; i < ARRAY_SIZE(peripherals); i++) {
				...
				if (bt_hogp_assign_check(hogp) && bt_hogp_conn(hogp) == event->id) {
	...
	if (is_ble_peer_operation_event(aeh)) {
		const struct ble_peer_operation_event *event = cast_ble_peer_operation_event(aeh);
