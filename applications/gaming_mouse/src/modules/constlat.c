...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (IS_ENABLED(CONFIG_DESKTOP_CONSTLAT_DISABLE_ON_STANDBY) && is_power_down_event(aeh)) {
		constlat_off();
		return false;
	}

	if (IS_ENABLED(CONFIG_DESKTOP_CONSTLAT_DISABLE_ON_STANDBY) && is_wake_up_event(aeh)) {
		constlat_on();
		return false;
	}
