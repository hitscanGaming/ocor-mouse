...
static int store_bt_stack_id_lut(void)
{
	if (IS_ENABLED(CONFIG_SETTINGS)) {
		...
		int err = settings_save_one(key, bt_stack_id_lut, sizeof(bt_stack_id_lut));
		if (err) {
			LOG_ERR("Problem storing bt_stack_id_lut: (err = %d)", err);
			return err;
		}
	}
	return 0;
...
static void swap_bt_stack_peer_id(void)
{
	...
	if (IS_ENABLED(CONFIG_DESKTOP_BLE_USE_DEFAULT_ID)) {
		if ((bt_stack_id_lut[0] == BT_ID_DEFAULT) && (get_app_id() == 0)) {
...
static int shell_remove_peers(const struct shell *shell, size_t argc, char **argv)
{
	int err;
...
static void erase_adv_confirm(void)
{
	...
	if (err < 0) {
		LOG_ERR("Cannot reset id %u (err:%d)", get_bt_stack_peer_id(get_app_id()), err);
		module_set_state(MODULE_STATE_ERROR);
		return;
	}
...
static void select_ble_peers(void)
{
	...
	if (state == STATE_DONGLE) {
		...
		__ASSERT_NO_MSG((state == STATE_STANDBY) || (state == STATE_IDLE));
	}
...
static void handle_click(enum click click)
{
	if (click != CLICK_NONE) {
		for (size_t i = 0; i < ARRAY_SIZE(state_switch); i++) {
			if ((state_switch[i].state == state) && (state_switch[i].click == click)) {
				...
				if ((state != STATE_IDLE) && (state != STATE_DONGLE)) {
					...
					k_work_reschedule(&timeout, work_delay);
				} else {
...
{
	__ASSERT_NO_MSG((state == STATE_ERASE_PEER) || (state == STATE_ERASE_ADV) ||
			(state == STATE_DONGLE_ERASE_PEER) || (state == STATE_DONGLE_ERASE_ADV) ||
			(state == STATE_SELECT_PEER));
...
static int init(void)
{
	...
	if (IS_ENABLED(CONFIG_DESKTOP_BT_PERIPHERAL) && !storage_data_is_valid()) {
...
static bool ble_peer_event_handler(const struct ble_peer_event *event)
{
	...
	if (event->state == PEER_STATE_CONNECTED) {
		...
		if ((remains < ERASE_ADV_NEW_CONN_TIMEOUT_MS) && !erase_adv_was_extended) {
			k_work_reschedule(&timeout, ERASE_ADV_NEW_CONN_TIMEOUT);
		...
		if (bt_info.id != get_bt_stack_peer_id(get_app_id())) {
			...
			int err = bt_conn_disconnect(event->id, BT_HCI_ERR_REMOTE_USER_TERM_CONN);
		...
		struct ble_peer_operation_event *new_event = new_ble_peer_operation_event();
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (IS_ENABLED(CONFIG_DESKTOP_BT_PERIPHERAL) && is_ble_peer_event(aeh)) {
		return ble_peer_event_handler(cast_ble_peer_event(aeh));
	}

	if (IS_ENABLED(CONFIG_DESKTOP_BLE_PEER_CONTROL) && is_click_event(aeh)) {
		return click_event_handler(cast_click_event(aeh));
	}

	if (IS_ENABLED(CONFIG_DESKTOP_BLE_DONGLE_PEER_ENABLE) && is_selector_event(aeh)) {
		selector_event_handler(cast_selector_event(aeh));
		return false;
	}

	if (IS_ENABLED(CONFIG_CAF_POWER_MANAGER) && is_power_down_event(aeh)) {
		return handle_power_down_event(cast_power_down_event(aeh));
	}

	if (IS_ENABLED(CONFIG_CAF_POWER_MANAGER) && is_wake_up_event(aeh)) {
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	sub_peers, SHELL_CMD_ARG(show, NULL, "Show bonded peers", shell_show_peers, 0, 0),
	SHELL_CMD_ARG(remove, NULL, "Remove bonded devices", shell_remove_peers, 0, 0),
