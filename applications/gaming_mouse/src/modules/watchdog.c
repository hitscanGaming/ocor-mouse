...
{
	struct wdt_data_storage *data = CONTAINER_OF(work_desc, struct wdt_data_storage, work.work);
	...
	if (err) {
		...
		k_work_reschedule(&data->work, K_MSEC(WDT_FEED_WORKER_DELAY_MS));
	}
}
...
static int watchdog_timeout_install(struct wdt_data_storage *data)
{
	static const struct wdt_timeout_cfg wdt_settings = {
		.window = {.min = 0, .max = CONFIG_DESKTOP_WATCHDOG_TIMEOUT},
		.callback = NULL,
		.flags = WDT_FLAG_RESET_SOC};

	__ASSERT_NO_MSG(data != NULL);

	data->wdt_channel_id = wdt_install_timeout(data->wdt, &wdt_settings);
