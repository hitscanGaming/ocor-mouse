...
#define INPUT_REPORT_DATA_COUNT                                                                    \
...
#define INPUT_REPORT_STATE_COUNT                                                                   \
	(IS_ENABLED(CONFIG_DESKTOP_HID_REPORT_MOUSE_SUPPORT) +                                     \
	 IS_ENABLED(CONFIG_DESKTOP_HID_REPORT_KEYBOARD_SUPPORT) +                                  \
	 IS_ENABLED(CONFIG_DESKTOP_HID_REPORT_SYSTEM_CTRL_SUPPORT) +                               \
	 IS_ENABLED(CONFIG_DESKTOP_HID_REPORT_CONSUMER_CTRL_SUPPORT) +                             \
...
#define ITEM_COUNT                                                                                 \
	MAX(MAX(MOUSE_REPORT_BUTTON_COUNT_MAX, KEYBOARD_REPORT_KEY_COUNT_MAX),                     \
	    MAX(SYSTEM_CTRL_REPORT_KEY_COUNT_MAX, CONSUMER_CTRL_REPORT_KEY_COUNT_MAX))
...
static const struct report_data empty_rd = {.eventq.root =
						    SYS_SLIST_STATIC_INIT(&empty_rd.eventq.root)};
...
static bool report_send(struct report_state *rs, struct report_data *rd, bool check_state,
			bool send_always);
...
{
	struct hid_keymap key = {.key_id = key_id};

	struct hid_keymap *map =
		bsearch(&key, hid_keymap, ARRAY_SIZE(hid_keymap), sizeof(key), hid_keymap_compare);
...
static void eventq_region_purge(struct eventq *eventq, sys_snode_t *last_to_purge)
{
	sys_snode_t *tmp;
...
static void eventq_cleanup(struct eventq *eventq, uint32_t timestamp)
{
	...
	SYS_SLIST_FOR_EACH_NODE(&eventq->root, first_valid) {
		uint32_t diff =
			timestamp - CONTAINER_OF(first_valid, struct item_event, node)->timestamp;
	...
	SYS_SLIST_FOR_EACH_NODE_SAFE(&eventq->root, cur, tmp_safe) {
		const struct item cur_item = CONTAINER_OF(cur, struct item_event, node)->item;
		...
		if (cur_item.value > 0) {
			...
			SYS_SLIST_ITERATE_FROM_NODE(&eventq->root, j)
			{
				...
				const struct item item =
					CONTAINER_OF(j, struct item_event, node)->item;
...
static struct report_state *get_report_state(struct subscriber *subscriber, uint8_t report_id)
{
...
static bool key_value_set(struct items *items, uint16_t usage_id, int16_t value)
{
	...
	p_item = bsearch(&i, items->item, ARRAY_SIZE(items->item), sizeof(items->item[0]),
			 usage_id_compare);
...
static void send_report_keyboard(struct report_state *rs, struct report_data *rd)
{
	...
	struct hid_report_event *event =
		new_hid_report_event(sizeof(rs->report_id) + REPORT_SIZE_KEYBOARD_KEYS);
	event->source = &state;
	...
	for (size_t i = 0; (i < max) && (cnt < KEYBOARD_REPORT_KEY_COUNT_MAX); i++) {
		...
		if (item.usage_id) {
			...
			if (item.usage_id <= KEYBOARD_REPORT_LAST_KEY) {
				...
				BUILD_ASSERT(KEYBOARD_REPORT_LAST_MODIFIER -
						     KEYBOARD_REPORT_FIRST_MODIFIER <
					     8);
...
static void send_report_mouse(struct report_state *rs, struct report_data *rd)
{
	...
	/* Wheel */
	int16_t wheel = MAX(MIN(rd->axes.axis[MOUSE_REPORT_AXIS_WHEEL], MOUSE_REPORT_WHEEL_MAX),
			    MOUSE_REPORT_WHEEL_MIN);
	if (wheel) {
		rd->axes.axis[MOUSE_REPORT_AXIS_WHEEL] -= wheel;
	}
	...
	struct hid_report_event *event =
		new_hid_report_event(sizeof(rs->report_id) + REPORT_SIZE_MOUSE);
	...
	event->dyndata.data[4] = x_buff[1] & 0xFF;
	event->dyndata.data[5] = y_buff[0];
	event->dyndata.data[6] = y_buff[1] & 0xFF;
...
static void send_report_boot_mouse(struct report_state *rs, struct report_data *rd)
{
	...
	size_t report_size = sizeof(rs->report_id) + sizeof(dx) + sizeof(dy) + sizeof(button_bm);
...
static void send_report_ctrl(struct report_state *rs, struct report_data *rd)
{
	...
	__ASSERT_NO_MSG(report_size == sizeof(rs->report_id) + sizeof(rd->items.item[0].usage_id));
	event->dyndata.data[0] = rs->report_id;

	const size_t idx = ARRAY_SIZE(rd->items.item) - 1;

	sys_put_le16(rd->items.item[idx].usage_id, &event->dyndata.data[sizeof(rs->report_id)]);
...
static bool update_report(struct report_data *rd)
{
	...
	while (!update_needed && !eventq_is_empty(&rd->eventq)) {
		...
		update_needed = key_value_set(&rd->items, event->item.usage_id, event->item.value);
			...
			bool send_always)
{
	...
	if (!rs || !rd) {
		if (!rs) {
			LOG_WRN("No linked report state");
		}
		if (!rd) {
			LOG_WRN("No linked report data");
		}
...
static void report_issued(const void *subscriber_id, uint8_t report_id, bool error)
{
	...
	if (rs->state != STATE_DISCONNECTED) {
		...
		if (error && (rs->linked_rd->linked_rs == rs)) {
	...
	while (true) {
		if ((next_rs->state != STATE_DISCONNECTED) && (next_rs->cnt == 0) &&
						   ...
						   struct report_state *former_rs)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(state.subscriber); i++) {
		...
		for (size_t j = 0; j < ARRAY_SIZE(sub->state); j++) {
			...
			if (((rs->linked_rd == rd) || ((rs->linked_rd == &empty_rd) &&
						       (get_used_rd(rs->report_id) == rd))) &&
			    (sub_prio_result <= sub_prio) && (rs != former_rs)) {
...
static void connect(const void *subscriber_id, uint8_t report_id)
{
	...
	LOG_INF("Connect subscriber %p", subscriber_id);
...
static void connect_subscriber(const void *subscriber_id, uint8_t priority, uint8_t pipeline_size,
			       uint8_t report_max)
{
...
static void disconnect_subscriber(const void *subscriber_id)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(s->state); i++) {
		...
		if (rs->linked_rd) {
			...
			__ASSERT_NO_MSG((rs->report_id > 0) && (rs->report_id < REPORT_ID_COUNT));
			if (rd->linked_rs == rs) {
				clear_report_data(rd);
				rs->linked_rd = NULL;
...
static void enqueue(struct report_data *rd, uint16_t usage_id, int16_t value, bool connected)
{
	eventq_cleanup(&rd->eventq, k_uptime_get_32());
	...
	if (eventq_is_full(&rd->eventq)) {
		if (!connected) {
			...
			SYS_SLIST_FOR_EACH_NODE(&rd->eventq.root, i) {
				 ...
				 */
				uint32_t timestamp =
					CONTAINER_OF(i, struct item_event, node)->timestamp +
...
static void init(void)
{
	...
	if (IS_ENABLED(CONFIG_DESKTOP_HID_REPORT_CONSUMER_CTRL_SUPPORT)) {
		...
		state.report_data[data_id].items.item_count_max =
...
static void handle_hid_output_report(struct subscriber *sub, uint8_t report_id, const uint8_t *data,
				     size_t len)
{
	__ASSERT_NO_MSG(sub);
...
{
	if (!IS_ENABLED(CONFIG_DESKTOP_MOTION_NONE) && is_motion_event(aeh)) {
	...
	if (IS_ENABLED(CONFIG_DESKTOP_WHEEL_ENABLE) && is_wheel_event(aeh)) {
		return handle_wheel_event(cast_wheel_event(aeh));
	}

	if (IS_ENABLED(CONFIG_CAF_BUTTON_EVENTS) && is_button_event(aeh)) {
		return handle_button_event(cast_button_event(aeh));
	}

	if (IS_ENABLED(CONFIG_DESKTOP_HID_REPORT_KEYBOARD_SUPPORT) && is_hid_report_event(aeh)) {
