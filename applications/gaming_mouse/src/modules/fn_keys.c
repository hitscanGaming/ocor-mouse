...
static int settings_set(const char *key, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!strcmp(key, FN_LOCK_STORAGE_NAME)) {
		ssize_t len = read_cb(cb_arg, &fn_lock_active, sizeof(fn_lock_active));
...
SETTINGS_STATIC_HANDLER_DEFINE(fn_keys, MODULE_NAME, NULL, settings_set, NULL, NULL);
...
static bool fn_key_enabled(uint16_t key_id)
{
	...
	uint16_t *p =
		bsearch(&key_id, fn_keys, ARRAY_SIZE(fn_keys), sizeof(fn_keys[0]), key_id_compare);
...
static void store_fn_lock(void)
{
	if (IS_ENABLED(CONFIG_DESKTOP_STORE_FN_LOCK)) {
		...
		int err = settings_save_one(key, &fn_lock_active, sizeof(fn_lock_active));
...
static bool button_event_handler(const struct button_event *event)
{
	...
	if (event->pressed && fn_active) {
		...
		new_event->key_id = FN_KEY_ID(KEY_COL(event->key_id), KEY_ROW(event->key_id));
	...
	if (!event->pressed) {
		...
		if (key_was_remapped) {
			...
			new_event->key_id =
				FN_KEY_ID(KEY_COL(event->key_id), KEY_ROW(event->key_id));
			APP_EVENT_SUBMIT(new_event);
