...
SETTINGS_STATIC_HANDLER_DEFINE(ble_scan, MODULE_NAME, NULL, settings_set, verify_subscribed_peers,
			       NULL);
...
static void update_work(enum state prev_state, enum state new_state)
{
	...
	case STATE_INACTIVE:
		/* Schedule work only if state transition is possible. */
		if (update_state_transition(STATE_INACTIVE, STATE_ACTIVE, false) == STATE_ACTIVE) {
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (IS_ENABLED(CONFIG_DESKTOP_BLE_SCAN_PM_EVENTS) && is_power_down_event(aeh)) {
		return handle_power_down_event(cast_power_down_event(aeh));
	}

	if (IS_ENABLED(CONFIG_DESKTOP_BLE_SCAN_PM_EVENTS) && is_wake_up_event(aeh)) {
