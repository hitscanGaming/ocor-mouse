...
#define _DEFINE_FREQ_MASK_BIT(name, freq_suffix)                                                   \
	IF_ENABLED(CONFIG_DESKTOP_DVFS_STATE_##name##_##freq_suffix, (BIT(DVFS_STATE_##name) |))
...
static const struct dvfs_frequency dvfs_freq_array[] = {
	...
	{.freq = CONFIG_DESKTOP_DVFS_FREQ_LOW, .bitmask = UINT8_MAX},
};
...
static void dvfs_notify_cb(struct onoff_manager *srv, struct onoff_client *cli, uint32_t state,
			   int res)
{
...
static void dvfs_frequency_update(void)
{
	...
	if ((!k_work_delayable_is_pending(&dvfs_retry.retry_work)) && !request_in_progress &&
	    (required_freq != current_freq)) {
...
static void dvfs_state_timeout_work_handler(struct k_work *work)
{
	...
	struct dvfs_state_timeout *timeout_data =
		CONTAINER_OF(dwork, struct dvfs_state_timeout, timeout_work);
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (is_module_state_event(aeh)) {
		...
		if (check_state(event, MODULE_ID(main), MODULE_STATE_READY)) {
			...
			__ASSERT_NO_MSG((dvfs_freq_array[0].bitmask & dvfs_freq_array[1].bitmask) ==
					0);
	...
	if (IS_ENABLED(CONFIG_DESKTOP_DVFS_STATE_LLPM_CONNECTED_ENABLE) && is_ble_peer_event(aeh)) {
	...
	if (IS_ENABLED(CONFIG_DESKTOP_DVFS_STATE_USB_CONNECTED_ENABLE) && is_usb_state_event(aeh)) {
