...
static struct bt_uuid_128 custom_desc_uuid =
	BT_UUID_INIT_128(BT_UUID_128_ENCODE(0x24470275, 0x4619, 0x177d, 0xd578, 0xb698e6e6654c));

static const struct bt_uuid_128 custom_desc_chrc_uuid =
	BT_UUID_INIT_128(BT_UUID_128_ENCODE(0x25470275, 0x4619, 0x177d, 0xd578, 0xb698e6e6654c));

static bool active;
static uint8_t chmap[CHMAP_BLE_BITMASK_SIZE];
static struct k_spinlock lock;

static void chmap_ccc_cfg_changed(const struct bt_gatt_attr *attr, uint16_t value)
{
	active = (value == BT_GATT_CCC_NOTIFY);
}

static ssize_t read_chmap(struct bt_conn *conn, const struct bt_gatt_attr *attr, void *buf,
			  uint16_t len, uint16_t offset)
{
	...
	return bt_gatt_attr_read(conn, attr, buf, len, offset, chmap_local, sizeof(chmap_local));
}

BT_GATT_SERVICE_DEFINE(qos_svc, BT_GATT_PRIMARY_SERVICE(&custom_desc_uuid),
		       BT_GATT_CHARACTERISTIC(&custom_desc_chrc_uuid.uuid,
					      BT_GATT_CHRC_READ | BT_GATT_CHRC_NOTIFY,
					      BT_GATT_PERM_READ_ENCRYPT, read_chmap, NULL, &chmap),
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	if (is_ble_qos_event(aeh)) {
		...
		if (active) {
			int err = bt_gatt_notify(NULL, &qos_svc.attrs[1], &event->chmap,
						 sizeof(event->chmap));
