...
BT_HIDS_DEF(
	...
	IF_ENABLED(CONFIG_DESKTOP_HID_REPORT_MOUSE_SUPPORT,
		   (REPORT_SIZE_MOUSE, )) IF_ENABLED(CONFIG_DESKTOP_HID_REPORT_KEYBOARD_SUPPORT,
				...
				IF_ENABLED(
					...
					(REPORT_SIZE_USER_CONFIG, /* HID feature report. */
					 IF_ENABLED(
						 ...
						 /* HID output report. */
						 (REPORT_SIZE_USER_CONFIG, )))) 0 /* Appease macro
										     with a dummy
										     zero */
);
...
{
	bool boot = (report_id == REPORT_ID_BOOT_MOUSE) || (report_id == REPORT_ID_BOOT_KEYBOARD);
...
static void feature_report_handler(struct bt_hids_rep *rep, struct bt_conn *conn, bool write)
{
	if (IS_ENABLED(CONFIG_DESKTOP_CONFIG_CHANNEL_ENABLE)) {
		if (!write) {
			int err = config_channel_transport_get(&cfg_chan_transport, rep->data,
							       rep->size);
			...
			int err = config_channel_transport_set(&cfg_chan_transport, rep->data,
							       rep->size);
...
static int module_init(void)
{
	...
	/* Declare HID reports */
	struct bt_hids_inp_rep *input_report = &hids_init_param.inp_rep_group_init.reports[0];
	...
	if (IS_ENABLED(CONFIG_DESKTOP_HID_BOOT_INTERFACE_MOUSE)) {
		...
		hids_init_param.boot_mouse_notif_handler = boot_mouse_notif_handler;
	}
...
{
	static void (*const report_sent_cb[REPORT_ID_COUNT])(struct bt_conn *conn,
							     void *user_data) = {
	...
	case REPORT_ID_BOOT_MOUSE:
		if (!protocol_boot) {
			...
			err = bt_hids_boot_mouse_inp_rep_send(&hids_obj, cur_conn, &buffer[0],
							      buffer[1], buffer[2],
	...
	case REPORT_ID_BOOT_KEYBOARD:
		if (!protocol_boot) {
			...
			err = bt_hids_boot_kb_inp_rep_send(&hids_obj, cur_conn, buffer, size,
							   report_sent_cb[report_id]);
		}
		break;
	default:
		if (protocol_boot) {
			...
			err = bt_hids_inp_rep_send(&hids_obj, cur_conn, report_index[report_id],
						   buffer, size, report_sent_cb[report_id]);
		}
		break;
	}
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (is_module_state_event(aeh)) {
		...
		if (check_state(event, MODULE_ID(main), MODULE_STATE_READY)) {
			...
			if (CONFIG_DESKTOP_HIDS_FIRST_REPORT_DELAY > 0) {
				k_work_init_delayable(&notify_secured, notify_secured_fn);
			}
