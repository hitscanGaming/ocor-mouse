...
#define SYNC_BUFFER_SIZE                                                                           \
	(CONFIG_DESKTOP_CONFIG_CHANNEL_DFU_SYNC_BUFFER_SIZE * sizeof(uint32_t)) /* bytes */
...
const static char *const opt_descr[] = {
	[DFU_OPT_START] = "start",    [DFU_OPT_DATA] = "data",
	[DFU_OPT_SYNC] = "sync",      [DFU_OPT_REBOOT] = "reboot",
	[DFU_OPT_FWINFO] = "fwinfo",  [DFU_OPT_VARIANT] = OPT_DESCR_MODULE_VARIANT,
...
static void background_erase_handler(struct k_work *work)
{
	...
	if (device_in_use) {
		device_in_use = false;
		k_work_reschedule(&background_erase, BACKGROUND_FLASH_ERASE_TIMEOUT);
		return;
	}
...
static void handle_dfu_start(const uint8_t *data, const size_t size)
{
	...
	LOG_INF("DFU start received img_length:%" PRIu32 " img_csum:0x%" PRIx32 " offset:%" PRIu32,
		length, csum, offset);

	if (offset != 0) {
		if ((offset != cur_offset) || (length != img_length) || (csum != img_csum)) {
			LOG_WRN("Cannot restart DFU"
				" length: %" PRIu32 " %" PRIu32 " csum: 0x%" PRIx32 " 0x%" PRIx32
				" offset: %" PRIu32 " %" PRIu32,
				length, img_length, csum, img_csum, offset, cur_offset);
	...
	if (err) {
		...
		LOG_WRN("Insufficient space for DFU (%zu < %" PRIu32 ")", flash_area->fa_size,
			img_length);
...
static void handle_dfu_sync(uint8_t *data, size_t *size)
{
	...
	size_t data_size = sizeof(dfu_state) + sizeof(img_length) + sizeof(img_csum) +
			   sizeof(cur_offset) + sizeof(sync_buffer_size);
	...
	*size = data_size;
	...
	if (can_access_flash && !k_work_delayable_is_pending(&dfu_timeout) &&
...
static void handle_image_info_request(uint8_t *data, size_t *size)
{
	...
	if (info) {
		...
		LOG_INF("Primary slot| image size:%" PRIu32 " major:%" PRIu8 " minor:%" PRIu8
			" rev:0x%" PRIx16 " build:0x%" PRIx32,
			image_size, major, minor, revision, build_num);

		size_t data_size = sizeof(flash_area_id) + sizeof(image_size) + sizeof(major) +
				   sizeof(minor) + sizeof(revision) + sizeof(build_num);
		size_t pos = 0;
...
static void handle_image_info_request(uint8_t *data, size_t *size)
{
	...
	int err = boot_read_bank_header(bank_header_area_id, &header, sizeof(header));

	if (!err) {
		LOG_INF("Primary slot| image size:%" PRIu32 " major:%" PRIu8 " minor:%" PRIu8
			" rev:0x%" PRIx16 " build:0x%" PRIx32,
			header.h.v1.image_size, header.h.v1.sem_ver.major,
			header.h.v1.sem_ver.minor, header.h.v1.sem_ver.revision,
			header.h.v1.sem_ver.build_num);

		size_t data_size = sizeof(flash_area_id) + sizeof(header.h.v1.image_size) +
...
static void handle_image_info_request(uint8_t *data, size_t *size)
{
	...
	if (!err) {
		err = suit_get_installed_manifest_info(&(class_info.class_id), &seq_num,
						       &version_raw, NULL, NULL, NULL);
	}
	if (!err) {
		...
		if (is_semver_supported) {
			err = suit_metadata_version_from_array(&version, version_raw.raw,
							       version_raw.len);
		}
	}
	...
	if (!err) {
		...
		if (is_semver_supported) {
			...
			if (release_type) {
				LOG_INF("Booted application version: %d.%d.%d-%s%d", version.major,
					version.minor, version.patch, release_type,
					version.pre_release_number);
			} else {
				LOG_INF("Booted application version: %d.%d.%d", version.major,
