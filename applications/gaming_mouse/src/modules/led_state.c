...
#include "usb_event.h"
#endif /* CONFIG_DESKTOP_USB_ENABLE */
...
static enum led_system_state system_state = LED_SYSTEM_STATE_NO_CHARGING;
...
static void load_system_state_led(void)
{
	...
	LOG_ERR("LED effect set to %d", system_state);
	APP_EVENT_SUBMIT(event);
}
...
static void set_system_state_led(enum led_system_state state)
{
	...
	if (system_state != LED_SYSTEM_STATE_ERROR && state != system_state) {
		system_state = state;
		load_system_state_led();
	}
}

#if IS_ENABLED(CONFIG_DESKTOP_ESB_RX_ENABLE) || IS_ENABLED(CONFIG_DESKTOP_ESB_TX_ENABLE)
static void set_peer_state_led(enum led_peer_state state)
{
	if (led_map[LED_ID_PEER_STATE] == LED_UNAVAILABLE) {
		return;
	}
	...
	event->led_id = led_map[LED_ID_PEER_STATE];
	// Assuming single peer index 0 for now as previously used logic was more complex
	event->led_effect = &led_peer_state_effect[0][state];
	APP_EVENT_SUBMIT(event);
}
#endif
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (is_low_battery_event(aeh)) {
		// struct low_battery_event *event = cast_low_battery_event(aeh);
		if (system_state == LED_SYSTEM_STATE_NO_CHARGING) {
			set_system_state_led(LED_SYSTEM_STATE_NEED_CHARGING);
		}
	...
	if (is_usb_state_event(aeh)) {
		...
		case USB_STATE_DISCONNECTED:
			set_system_state_led(LED_SYSTEM_STATE_NO_CHARGING);
			break;
...
#if IS_ENABLED(CONFIG_DESKTOP_ESB_RX_ENABLE)
	if (is_esb_state_event(aeh)) {
		struct esb_state_event *event = cast_esb_state_event(aeh);

		switch (event->state) {
		case ESB_STATE_ACTIVE:
			set_peer_state_led(LED_PEER_STATE_CONNECTED);
			break;
		case ESB_STATE_PAIRING:
			set_peer_state_led(LED_PEER_STATE_PEER_SEARCH);
			break;
		case ESB_STATE_DISCONNECTED:
			set_peer_state_led(LED_PEER_STATE_DISCONNECTED);
			break;
		default:
			break;
		}
...
APP_EVENT_SUBSCRIBE(MODULE, low_battery_event);
#if IS_ENABLED(CONFIG_DESKTOP_ESB_RX_ENABLE) || IS_ENABLED(CONFIG_DESKTOP_ESB_TX_ENABLE)
APP_EVENT_SUBSCRIBE(MODULE, esb_state_event);
#endif
