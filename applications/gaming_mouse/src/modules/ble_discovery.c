...
static void hids_discovery_completed(struct bt_gatt_dm *dm, void *context)
{
	...
	struct ble_discovery_complete_event *event = new_ble_discovery_complete_event();
...
static void read_dev_descr_hwid(const uint8_t *ptr, uint16_t len)
{
	...
	int pos = 0;
	int err = snprintf(&log_buf[pos], log_buf_len - pos, "%02x", ptr[0]);

	for (size_t i = 1; (i < len) && (err > 0); i++) {
		pos += err;
		err = snprintf(&log_buf[pos], log_buf_len - pos, "%02x", ptr[i]);
	}
...
static uint8_t read_attr(struct bt_conn *conn, uint8_t err, struct bt_gatt_read_params *params,
...
static void start_discovery(struct bt_conn *conn, const struct bt_uuid *svc_uuid,
			    bool discovery_hids)
{
	...
	int err = bt_gatt_dm_start(conn, svc_uuid,
				   discovery_hids ? &hids_discovery_cb : &discovery_cb, NULL);
