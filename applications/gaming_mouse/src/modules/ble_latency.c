...
#define SECURITY_FAIL_TIMEOUT_MS      K_SECONDS(CONFIG_DESKTOP_BLE_SECURITY_FAIL_TIMEOUT_S)
...
{
	const struct bt_le_conn_param param = {.interval_min = REG_CONN_INTERVAL_BLE_DEFAULT,
...
static void set_conn_latency(bool low_latency)
{
	...
	uint16_t interval = (info.le.interval & REG_CONN_INTERVAL_LLPM_MASK)
				    ? REG_CONN_INTERVAL_BLE_DEFAULT
				    : info.le.interval;
	const struct bt_le_conn_param param = {.interval_min = interval,
...
{
	if (!IS_ENABLED(CONFIG_DESKTOP_BLE_LOW_LATENCY_LOCK) || !(latency_state & CONN_IS_LLPM)) {
		return;
	}
	...
	if (latency_state & CONN_LOW_LATENCY_LOCKED) {
		...
		k_work_reschedule(&low_latency_check, LOW_LATENCY_CHECK_PERIOD_MS);
	}
}
...
{
	__ASSERT_NO_MSG(
		!((latency_state & CONN_LOW_LATENCY_LOCKED) && (latency_state & CONN_IS_LLPM)));
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (is_ble_peer_event(aeh)) {
		...
		case PEER_STATE_CONNECTED:
			...
			k_work_reschedule(&security_timeout, SECURITY_FAIL_TIMEOUT_MS);
			break;
	...
	if (IS_ENABLED(CONFIG_DESKTOP_BLE_LOW_LATENCY_LOCK) &&
	    IS_ENABLED(CONFIG_DESKTOP_BLE_LATENCY_PM_EVENTS) && is_power_down_event(aeh)) {
	...
	if (IS_ENABLED(CONFIG_DESKTOP_BLE_LOW_LATENCY_LOCK) &&
	    IS_ENABLED(CONFIG_DESKTOP_BLE_LATENCY_PM_EVENTS) && is_wake_up_event(aeh)) {
		latency_state |= CONN_LOW_LATENCY_LOCKED;
		update_llpm_conn_latency_lock();
