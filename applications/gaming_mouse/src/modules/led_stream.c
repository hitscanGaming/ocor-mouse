...
{
	size_t len = (STEPS_QUEUE_ARRAY_SIZE + led->rx_idx - led->tx_idx) % STEPS_QUEUE_ARRAY_SIZE;
...
static bool store_data(const uint8_t *data, const size_t size, struct led *led)
{
	...
	if (free_places > 0) {
		LOG_DBG("Insert data on position %" PRIu8 ", free places %zu", led->rx_idx,
			free_places);
...
static void fetch_leds_state(uint8_t *data, size_t *size)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(leds); i++) {
		...
		LOG_DBG("Free places left: %" PRIu8 " for led: %zu", free_places, i);
...
{
	GEN_CONFIG_EVENT_HANDLERS(STRINGIFY(MODULE), opt_descr, config_set, config_get);
