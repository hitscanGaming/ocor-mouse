...
#define THREAD_STACK_SIZE                                                                          \
	(CONFIG_DESKTOP_BLE_QOS_STACK_SIZE + CONFIG_DESKTOP_BLE_QOS_STATS_PRINT_STACK_SIZE)
#else
...
BUILD_ASSERT(sizeof(struct bt_hci_cp_le_set_host_chan_classif) == sizeof(struct params_chmap));
...
static const char *const opt_descr[] = {[BLE_QOS_OPT_BLACKLIST] = "blacklist",
...
static void update_parameters(const uint8_t *qos_ble_params, const uint8_t *qos_wifi_params)
{
	size_t pos;
	...
	if (qos_ble_params != NULL) {
		...
		filter_params.maintenance_sample_count = sys_get_le16(&qos_ble_params[pos]);
		...
		filter_params.ble_weight_crc_ok = sys_get_le16(&qos_ble_params[pos]);
		pos += sizeof(filter_params.ble_weight_crc_ok);

		filter_params.ble_weight_crc_error = sys_get_le16(&qos_ble_params[pos]);
		pos += sizeof(filter_params.ble_weight_crc_error);

		filter_params.ble_block_threshold = sys_get_le16(&qos_ble_params[pos]);
		...
		filter_params.eval_duration = sys_get_le16(&qos_ble_params[pos]);
		pos += sizeof(filter_params.eval_duration);

		filter_params.eval_keepout_duration = sys_get_le16(&qos_ble_params[pos]);
		pos += sizeof(filter_params.eval_keepout_duration);

		filter_params.eval_success_threshold = sys_get_le16(&qos_ble_params[pos]);
	...
	if (qos_wifi_params != NULL) {
		...
		filter_params.wifi_rating_inc = sys_get_le16(&qos_wifi_params[pos]);
		pos += sizeof(filter_params.wifi_rating_inc);

		filter_params.wifi_present_threshold = sys_get_le16(&qos_wifi_params[pos]);
		pos += sizeof(filter_params.wifi_present_threshold);

		filter_params.wifi_active_threshold = sys_get_le16(&qos_wifi_params[pos]);
...
SETTINGS_STATIC_HANDLER_DEFINE(ble_qos, MODULE_NAME, NULL, settings_set, NULL, NULL);
...
static void ble_chn_stats_print(bool update_channel_map)
{
	...
	if (IS_ENABLED(CONFIG_UART_LINE_CTRL)) {
		...
		err = uart_line_ctrl_get(cdc_dev, UART_LINE_CTRL_DTR, &cdc_val);
		if (!err) {
			cdc_dtr = cdc_val;
		}
	...
	if (update_channel_map) {
		str_len = snprintf(str, sizeof(str), "[%08" PRIu32 "]Channel map update\n",
	...
	for (uint8_t i = 0; i < CHMAP_BLE_CHANNEL_COUNT; i++) {
		chmap_filter_chn_info_get(chmap_inst, i, &chn_state, &chn_rating, &chn_freq);
		part_len = snprintf(&str[str_len], sizeof(str) - str_len,
				    "%" PRIu8 ":%" PRIu8 ":%" PRId16 ",", chn_freq, chn_state,
				    chn_rating);
		str_len += part_len;
...
static void hid_pkt_stats_print(uint32_t ble_recv)
{
	...
	time_diff = prev_timestamp < timestamp ? (timestamp - prev_timestamp)
					       : (UINT32_MAX - prev_timestamp + timestamp);
	prev_timestamp = timestamp;
	rate_diff =
		prev_ble <= ble_recv ? (ble_recv - prev_ble) : (UINT32_MAX - prev_ble + ble_recv);
	prev_ble = ble_recv;
	ble_rate = ROUNDED_DIV((rate_diff * CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC), time_diff);

	str_len = snprintf(str, sizeof(str), "[%08" PRIu32 "]Rate:%04" PRIu32 "\n", timestamp,
			   ble_rate);
	if (str_len <= 0 || str_len > sizeof(str)) {
		LOG_ERR("Encoding error");
		return;
...
static bool on_vs_evt(struct net_buf_simple *buf)
{
	...
	case SDC_HCI_SUBEVENT_VS_QOS_CONN_EVENT_REPORT:
		...
		chmap_filter_crc_update(chmap_inst, evt->channel_index, evt->crc_ok_count,
					evt->crc_error_count);
		return true;
	default:
		return false;
...
static void store_config(const uint8_t opt_id, const uint8_t *data, const size_t data_size)
{
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (is_module_state_event(aeh)) {
		...
		if (check_state(event, MODULE_ID(main), MODULE_STATE_READY)) {
			...
			chmap_inst = (struct chmap_instance *)chmap_instance_buf;
			err = chmap_filter_instance_init(chmap_inst, sizeof(chmap_instance_buf));
			if (err) {
			...
			LOG_DBG("Chmap lib version: %s", chmap_filter_version());
			...
			k_thread_create(&thread, thread_stack, THREAD_STACK_SIZE,
					(k_thread_entry_t)ble_qos_thread_fn, NULL, NULL, NULL,
...
static void ble_qos_thread_fn(void)
{
	while (true) {
		...
		uint16_t blacklist_update = (uint16_t)atomic_set(&new_blacklist, INVALID_BLACKLIST);

		if (blacklist_update != INVALID_BLACKLIST) {
			err = chmap_filter_blacklist_set(chmap_inst, blacklist_update);
			if (err) {
