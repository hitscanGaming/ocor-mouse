...
/* User Configuration Data Buffer */
/* Size should match REPORT_SIZE_USER_CONFIG (29) plus margin for Report ID if sent */
volatile uint8_t  UserConfigData[ FEATURE_REPORT_SIZE ]; 
volatile uint8_t  UserConfigReceived = 0;
...
// extern uint8_t USBHS_EP3_TX_Buf[ DEF_USB_EP2_HS_SIZE ];
// extern uint8_t USBHS_EP3_TX_Buf[ 8 ];
/* USB IN Endpoint Busy Flag */
volatile uint8_t  USBHS_Endp_Busy[ DEF_UEP_NUM ] = {0x00};
...
void USBHS_RCC_Init( void )
{
    ...
    RCC_USBHSPLLCKREFCLKConfig( RCC_USBHSPLLCKREFCLK_4M );  // change 4M to 8M
...
//    USBHSD->UEP3_TX_LEN =  8 ;
    USBHSD->UEP3_TX_CTRL = USBHS_UEP_T_RES_NAK;
    USBHS_Endp_Busy[ DEF_UEP3 ] = 0;
}
...
//                        USBHSD->UEP3_TX_DMA = (uint32_t)(uint8_t *)USBHS_EP3_TX_Buf;
                        ...
                        if( intst & USBHS_UIS_TOG_OK )
                        {
                            if( ( USBHS_SetupReqType & USB_REQ_TYP_MASK ) != USB_REQ_TYP_STANDARD )
                            {
                                if( ( USBHS_SetupReqType & USB_REQ_TYP_MASK ) == USB_REQ_TYP_CLASS )
                                {
                                    ...
                                    {
                                        case HID_SET_REPORT:
                                            // Handle User Config Data (Report ID 6)
                                            // Check low byte of wValue for Report ID
                                            if( (USBHS_SetupReqValue & 0xFF) == REPORT_ID_USER_CONFIG )
                                            {
                                                // USBHS_SetupReqLen contains the length of the data payload
                                                // Prevent buffer overflow
                                                uint8_t copy_len = (USBHS_SetupReqLen > sizeof(UserConfigData)) ? sizeof(UserConfigData) : USBHS_SetupReqLen;
                                                memcpy((void*)UserConfigData, USBHS_EP0_Buf, copy_len);
                                                
                                                // Set flag to notify main loop
                                                UserConfigReceived = 1;
                                            }
                                            // KB_LED_Cur_Status = USBHS_EP0_Buf[ 0 ]; // Original keyboard LED logic
                                            USBHS_SetupReqLen = 0; // Trigger Status Phase (ACK)
                                            break;
            ...
            case USBHS_UIS_TOKEN_SOF:
                if ( USBHS_DevEnumStatus && new_rx_data == 0x01) {
                    // printf("new_rx_data \r\n");
                    if( ( USBHS_Endp_Busy[ DEF_UEP3 ] & DEF_UEP_BUSY ) == 0x00 ) {
                        ...
                        // printf("no USBHS_Endp_Busy \r\n");
                        // volatile uint8_t *p = RB_Pop(&packetVal);
                        // if (p != NULL) {
                        //     memcpy( USBHSD_UEP_TXBUF (DEF_UEP3), (uint8_t*)p, MOUSE_REPORT_SIZE );
                        //     USBHS_Endp_Busy[ DEF_UEP3 ] |= DEF_UEP_BUSY;
                        //     /* endpoint n response tx ack */
                        //     USBHSD_UEP_TLEN( DEF_UEP3 ) = MOUSE_REPORT_SIZE;
                        //     USBHSD_UEP_TXCTRL( DEF_UEP3 ) = ( USBHSD_UEP_TXCTRL( DEF_UEP3 ) &= ~USBHS_UEP_T_RES_MASK ) | USBHS_UEP_T_RES_ACK;
                ...
                //         // memcpy( USBHSD_UEP_TXBUF (DEF_UEP3), KB_Data, 8 );
                //         // /* set endpoint busy */
                //         // USBHS_Endp_Busy[ DEF_UEP3 ] |= DEF_UEP_BUSY;
                //         // /* endpoint n response tx ack */
                //         // USBHSD_UEP_TLEN( DEF_UEP3 ) = 8;
                //         // USBHSD_UEP_TXCTRL( DEF_UEP3 ) = ( USBHSD_UEP_TXCTRL( DEF_UEP3 ) &= ~USBHS_UEP_T_RES_MASK ) | USBHS_UEP_T_RES_ACK;
    ...
    else if( intflag & USBHS_UIF_SETUP_ACT )
    {
            ...
            if( ( USBHS_SetupReqType & USB_REQ_TYP_MASK ) == USB_REQ_TYP_CLASS )
            {
                ...
                switch( USBHS_SetupReqCode )
                {
                    case HID_GET_REPORT:
                        if( (USBHS_SetupReqValue & 0xFF) == REPORT_ID_USER_CONFIG )
                        {
                            uint8_t copy_len = (USBHS_SetupReqLen > sizeof(UserConfigData)) ? 
                                            sizeof(UserConfigData) : USBHS_SetupReqLen;
                            if (spi_response_ready) {
                                memcpy(USBHS_EP0_Buf, (void*)UserConfigData, copy_len);
                        ...
                        break;
                    case HID_SET_REPORT:
                        // No logic needed here for Setup Phase.
                        // The default logic at the end of this block will setup UEP0_RX_CTRL 
                        // because USBHS_SetupReqLen (wLength) is > 0.
                        break;

                    case HID_SET_IDLE:
                        if( USBHS_SetupReqIndex == ITF_NUM_MOUSE1 )
                        {
                            USBHS_HidIdle[ 0 ] = (uint8_t)( USBHS_SetupReqValue >> 8 );
                        }
                        else if( USBHS_SetupReqIndex == ITF_NUM_MOUSE2 )
                        {
                    ...
                    case HID_GET_PROTOCOL:
                        ...
                        else if( USBHS_SetupReqIndex == ITF_NUM_MOUSE3 )
                        {
                            USBHS_EP0_Buf[ 0 ] = USBHS_HidProtocol[ 2 ];
                            len = 1;
                        }
                        else
                        {
                ...
                case USB_GET_DESCRIPTOR:
                    switch( (uint8_t)( USBHS_SetupReqValue >> 8 ) )
                    {
                        ...
                        case USB_DESC_TYPE_HID:
                            if( USBHS_SetupReqIndex == ITF_NUM_MOUSE1 )
                            {
                                pUSBHS_Descr = &MyCfgDescr[ OFFSET_HID_DESC_0 ];
                                len = USB_DESC_SIZE_HID;
                            }
                            else if( USBHS_SetupReqIndex == ITF_NUM_MOUSE2 )
                            {
                                pUSBHS_Descr = &MyCfgDescr[ OFFSET_HID_DESC_1 ];
                                len = USB_DESC_SIZE_HID;
                            }
                            else if( USBHS_SetupReqIndex == ITF_NUM_MOUSE3 )
                           {
                               pUSBHS_Descr = &MyCfgDescr[ OFFSET_HID_DESC_2 ];
                               len = USB_DESC_SIZE_HID;
                           }
                            else
                            {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    USBHSD->UEP0_TX_LEN = 0;
                    USBHSD->UEP0_TX_CTRL = USBHS_UEP_T_TOG_DATA1 | USBHS_UEP_T_RES_ACK;//发�?0长度数据�?? 给主�??
                }
                else
                {
                    USBHSD->UEP0_RX_CTRL = USBHS_UEP_R_TOG_DATA1 | USBHS_UEP_R_RES_ACK;//接收主机发�的OUT�??
                }
            }
        }
    ...
    // DMA_Tx_Init(DMA1_Channel5, (u32)&SPI2->DATAR, (u32)TxData, Size);
